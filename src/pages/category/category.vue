...
// 二级数据
const categoryChildList = computed(() => {
  return categoryTopList.value[currentIndex.value]?.children || []
})
// 轮播图数据
...
const getHomeBannerList = async () => {
  const res1 = await getHomeBannerAPI(2)
  bannerList.value = res1.result
}

onLoad(async () => {
  await Promise.all([getCategoryTopList(), getHomeBannerList()])
  ...
  <view class="viewport" v-if="isFinished">
    ...
    <view class="categories">
      ...
      <scroll-view class="secondary" scroll-y>
        ...
        <!-- 内容区域 -->
        <view class="panel" v-for="item in categoryChildList" :key="item.id">
          <view class="title">
            <text class="name">{{ item.name }}</text>
